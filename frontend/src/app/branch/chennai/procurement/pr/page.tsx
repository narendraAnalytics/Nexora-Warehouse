...
export default function BranchPRPage() {
  ...
  const wid     = WAREHOUSE_IDS[branch.toLowerCase()] ?? WAREHOUSE_IDS.chennai
  ...
  useEffect(() => {
    try {
      const raw = sessionStorage.getItem("chn_pr_analysis")
      if (!raw) return
  ...
  const generatePR = async () => {
    ...
    try {
      ...
      sessionStorage.removeItem("chn_pr_analysis")
    } catch (err: unknown) {
  ...
  return (
    <div className="nexora-pr">
      ...
      <div className="pr-topbar">
        ...
        <div>
          <div className="pr-tb-title">Procurement · Chennai</div>
