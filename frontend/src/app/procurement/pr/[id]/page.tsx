...
import "./procurement-pr.css"

interface PRItem {
  sku: string; name: string; category: string; brand: string
  current_qty: number; reorder_point: number
  agent_suggested_qty: number; manager_qty: number
  unit_cost: number; unit_price: number; line_total: number
  manager_comment: string
}

interface HistoryRow {
  id: string; action: string
  acted_by: string | null; acted_by_role: string | null
  notes: string | null; created_at: string
}

interface PRDetail {
  id: string; pr_number: string; workflow_id: string
  warehouse_id: string; status: string
  total_estimated_value: number; approval_level: string; approver_role: string
  requested_by: string; approved_by: string | null; approved_by_role: string | null
  rejection_reason: string | null; notes: string
  items: PRItem[]; inventory_analysis: Record<string, unknown>
  escalation_deadline: string; created_at: string
  approval_history: HistoryRow[]
}

interface FinanceAnalysis {
  pr_value: number; monthly_avg_spend: number; total_spend_3m: number
  impact_pct: number | null; recommendation: string
  approval_level: string; finance_records: number
}

type ModalAction = "approve" | "reject" | "changes" | "resubmit" | null

const WAREHOUSE_CITY: Record<string, string> = {
  "531e5c42-e4a1-4db0-a35c-a434f3b94344": "bangalore",
  "2521c486-9c74-49a2-8802-374dcb7d5d5d": "chennai",
}
...
function deadlineInfo(d: string): { label: string; urgent: boolean } {
  ...
  return { label: `${Math.floor(hrs / 24)}d ${hrs % 24}h remaining`, urgent: false }
}

function actionLabel(action: string): string {
  const map: Record<string, string> = {
    SUBMITTED: "Submitted", PENDING: "Pending Review",
    FINANCE_APPROVED: "Finance Approved", APPROVED: "Final Approved",
    REJECTED: "Rejected", CHANGES_REQUESTED: "Changes Requested", RESUBMITTED: "Resubmitted",
  }
...
// CEO sees full approval panel — finance first for L5, then CEO final
function approvalPanel(status: string, approverRole: string): "finance" | "ceo" | "resubmit" | "none" {
  const isL5 = approverRole === "CEO_AND_FINANCE"
  if (status === "PENDING") return isL5 ? "finance" : "ceo"
  if (status === "FINANCE_APPROVED") return "ceo"
  if (status === "CHANGES_REQUESTED" || status === "RESUBMITTED") return "resubmit"
  return "none"
}

export default function CEOPRDetailPage() {
  ...
  const fetchPR = useCallback(async () => {
    ...
    } catch { showToast("Failed to load PR") }
  ...
  useEffect(() => { fetchPR(); fetchFinance() }, [fetchPR, fetchFinance])
  ...
  async function handleAction(action: ModalAction) {
    ...
    try {
      ...
      if ((action === "approve" || action === "reject" || action === "changes") && panel === "finance") {
      ...
      if (!res.ok) {
        ...
        const msgs: Record<string, string> = {
          approve: "Approved successfully", reject: "Rejected",
          "request-changes": "Changes requested", resubmit: "Resubmitted for review",
        }
  ...
  const city = WAREHOUSE_CITY[pr.warehouse_id] ?? null
  ...
  return (
    <div className="nexora-ceo-pr">
      ...
      <div className="prd-topbar">
        ...
        <div className="prd-title">CEO Dashboard · PR Approval</div>
        <div className="prd-spacer"/>
        <button className="prd-back" onClick={() => router.push("/dashboard")} title="Back to CEO Dashboard" aria-label="Back to CEO Dashboard">
          ← CEO Dashboard
        </button>
        {city && (
          <button className="prd-back" onClick={() => router.push(`/branch/${city}/dashboard`)} title="Go to branch dashboard" aria-label="Go to branch dashboard" style={{ marginLeft: 6 }}>
            {city.charAt(0).toUpperCase() + city.slice(1)} Branch
          </button>
        )}
      </div>
      ...
      <div className="prd-body">
          ...
          <div className="prd-card">
            <div className="prd-header-row">
              <div style={{ flex: 1 }}>
                ...
                <div className="prd-wf-id">Workflow: {pr.workflow_id} · Requested by: {pr.requested_by}{city ? ` · ${city.charAt(0).toUpperCase() + city.slice(1)} Branch` : ""}</div>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                <span className={`prd-status-badge ${pr.status}`}>{pr.status.replace(/_/g, " ")}</span>
            ...
            <div className="prd-summary-row" style={{ marginTop: 14 }}>
              ...
              <div className="prd-sum-card">
                ...
                <div className="prd-sum-sub">{city ?? "branch"}</div>
              </div>
            </div>
          </div>
              ...
              <table className="prd-table">
                <thead>
                  <tr>
                    ...
                    <th>AI Suggested</th><th>Manager Qty</th>
                    <th>Unit Cost</th><th>Line Total</th>
                  </tr>
                </thead>
                <tbody>
                  {pr.items.map((item, i) => (
              ...
              {pr.approval_history.map((h, i) => (
                <div key={i} className="prd-tl-item">
                  ...
                  <div className="prd-tl-body">
                    ...
                    {h.acted_by && <div className="prd-tl-by">{h.acted_by} · {h.acted_by_role?.replace(/_/g, " ")}</div>}
        ...
        <div className="prd-side">
          ...
          {/* Finance Controller Panel (L5 PRs — first step) */}
          {panel === "finance" && (
            <div className="prd-card">
              <div className="prd-sec-head">Finance Controller Review</div>
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 12, lineHeight: 1.5 }}>
                This PR (₹{(pr.total_estimated_value / 1e7).toFixed(2)} Cr) requires Finance Controller approval before CEO final approval.
              </div>
          ...
          {panel === "ceo" && (
            <div className="prd-card">
              ...
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 12, lineHeight: 1.5 }}>
                {pr.status === "FINANCE_APPROVED"
                  ? "Finance analysis complete. Your approval converts this PR to APPROVED and triggers the Supplier Risk Agent."
                  : "Your CEO approval is required for this requisition."}
              </div>
              <div className="prd-actions">
                <button className="prd-action-btn approve" onClick={() => setModal("approve")}>
                  <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><polyline points="20 6 9 17 4 12"/></svg>
          ...
          {pr.status === "APPROVED" && (
            <div className="prd-card">
              ...
              <div style={{ fontSize: 11, color: "var(--muted)", marginBottom: 14, lineHeight: 1.6, textAlign: "center" }}>
                The branch manager will now run the Supplier Risk Agent and generate the Purchase Order from the branch dashboard.
              </div>
              {city && (
                <button
                  onClick={() => router.push(`/branch/${city}/dashboard`)}
                  style={{
                    width: "100%", padding: "9px 0", borderRadius: 8, border: "none",
                    background: "#18D8C3", color: "#0a1628", fontWeight: 800, fontSize: 12, cursor: "pointer",
                  }}
                  title={`Go to ${city} branch dashboard`}
                  aria-label={`Go to ${city} branch dashboard`}
                >
                  → {city.charAt(0).toUpperCase() + city.slice(1)} Branch Dashboard
                </button>
              )}
            </div>
          )}
        ...
        <div className="prd-modal-bg" onClick={(e) => e.target === e.currentTarget && setModal(null)}>
          <div className="prd-modal">
            ...
            <div className="prd-modal-sub">
              {modal === "approve" && panel === "finance" && "Approving as Finance Controller — PR will move to CEO for final approval."}
              {modal === "approve" && panel === "ceo" && "Giving final CEO approval — PR status will become APPROVED."}
